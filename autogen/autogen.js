...
const CONFIG = {
  ...
  // CAMINHOS CORRETOS DA SUA ESTRUTURA
  categories: [
  ...
  try {
    ...
    for (let f of files) {
      ...
      if (image && image.startsWith("/")) {
        ...
        image =
          `https://raw.githubusercontent.com/${CONFIG.owner}/${CONFIG.repo}/${CONFIG.branch}/${baseDir}/${image.replace(
            /^\.\/+/,
            ""
          )}`
            .replace(/\/+/g, "/")
        ...
        const baseDir = f.path.replace(/\/[^\/]+$/, "");
        image =
          `https://raw.githubusercontent.com/${CONFIG.owner}/${CONFIG.repo}/${CONFIG.branch}/${baseDir}/${image}`
            .replace(/\/+/g, "/")
            .replace("https:/", "https://");
      }
...
async function buildAllCategories() {
  ...
  try {
    localStorage.setItem("AUTOGEN_DATA", JSON.stringify(result));
  } catch (e) {}
